...
class LLMChat(BaseModel):
    ...
    def completion(
        self, message: Message | str = None, markdown: bool = True
    ) -> Markdown | str:
        message = (
        ...
        if not response.choices:
            raise ValueError(
                f"Did not receive a valid choice from model '{self.config.model}'"
            )

        # try update budget if budget is set
        if self.config.budget.is_on:
            self.config.budget.update_cost(self.config.budget.update_cost(response))
            self.config.budget.save()

        # parse and return response message, update existing messages
        resp_message = Message.model_validate(
